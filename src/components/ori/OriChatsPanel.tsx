...
export function OriChatsPanel({ conversations, loading, activeId, onSelect, onNewChat, onDelete, onClose }: OriChatsPanelProps) {
  ...
  return (
    <aside className="relative z-10 shrink-0 w-[280px] h-full border-l border-white/[.08] bg-[#0c0e13] flex flex-col">
      <div className="shrink-0 flex items-center justify-between px-4 py-4 border-b border-white/[.06]">
        <span className="text-[13px] font-semibold text-gray-200 tracking-tight">Chats</span>
        <div className="flex items-center gap-1">
          <button
            type="button"
            onClick={onNewChat}
            title="Nuevo chat"
            className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/[.06] transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {loading && (
          <p className="px-2.5 py-3 text-xs text-gray-600">Cargando…</p>
        )}

        {!loading && conversations.length === 0 && (
          <p className="px-2.5 py-3 text-xs text-gray-600 leading-relaxed">
        ...
        {GROUP_ORDER.filter(label => groups.has(label)).map(label => (
          <div key={label} className="mb-3">
            <p className="px-2.5 pb-1.5 pt-2 text-[10.5px] font-semibold uppercase tracking-wide text-gray-600">
              {label}
            </p>
            {groups.get(label)!.map(c => (
              <div
                ...
                className={`group flex items-center gap-1 rounded-lg px-2.5 py-2 cursor-pointer transition-colors ${
                  c.id === activeId ? "bg-[#0f7eff]/[.14] text-white" : "text-gray-400 hover:bg-white/[.05] hover:text-gray-200"
                }`}
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 p-1 rounded-md text-gray-600 opacity-0 group-hover:opacity-100 hover:text-red-400 hover:bg-white/[.08] transition-all"
                >
