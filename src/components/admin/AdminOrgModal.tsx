...
export interface AdminOrgFormValues {
  ...
  erp: boolean;
  /** Módulo Noova Seguros habilitado para esta organización. */
  seguros: boolean;
}
...
export function AdminOrgModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      setSeguros(initial?.seguros === true);
      setCopiedLink(false);
    }
  }, [open, initial]);
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="rounded-xl border border-white/[.08] bg-[#0d0e14]/80 p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <span>
                <span className="block text-sm font-medium text-white">Módulo Noova Seguros</span>
                <span className="block text-xs text-gray-500 mt-1 leading-relaxed">
                  Habilita «Noova Seguros» en el dashboard de esta organización: cotizador de autos y
                  conectores de aseguradoras (La Equidad).
                </span>
              </span>
            </label>
          </div>
