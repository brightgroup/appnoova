...
/** Silueta compartida por la familia de íconos (Ori y los agentes). */
...
export type OriIconVariant = "solid" | "hole" | "badge";
...
interface OriAnimatedIconProps extends SVGProps<SVGSVGElement> {
   ...
   * ojos rellenos según tema (.ori-eye-fill en globals.css) — para el ícono suelto, sin insignia propia,
   * sobre el fondo de la página (ej. el indicador de "pensando" dentro del chat).
   * "badge" = aro y orejas/antena en blanco fijo con ojos negros fijos — para cuando el ícono va DENTRO de
   * su propia insignia circular de fondo azul (encabezado, "Cotizar con ORI", tarjeta "Activa ORI Copiloto");
   * ahí un aro azul se pierde contra el azul de la insignia (bug reportado: "ni se ve el marco del muñequito"),
   * igual que el ícono blanco que ya usan los botones vecinos sobre el mismo fondo.
...
export function OriAnimatedIcon({ state = "idle", variant = "solid", ...props }: OriAnimatedIconProps) {
  ...
  const eyeFill = variant === "hole" ? "#000" : variant === "badge" ? "#000" : "currentColor";
  const eyeClassName = variant === "solid" ? "ori-eye-fill" : undefined;

  const eyes = (
    <g className={lookClass}>
      <g className={blinkClass}>
        <rect x="70" y="89" width="20" height="46" rx="10" fill={eyeFill} className={eyeClassName} />
        <rect x="110" y="89" width="20" height="46" rx="10" fill={eyeFill} className={eyeClassName} />
      </g>
    </g>
  );

  const ringFill = variant === "solid" ? "#0f7eff" : variant === "badge" ? "#fff" : "currentColor";
  ...
  return (
    <svg viewBox="0 0 200 200" className={breatheClass} {...props}>
      ...
      <g fill={ringFill} mask={variant === "hole" ? `url(#${maskId})` : undefined}>
        ...
        {(variant === "solid" || variant === "badge") && eyes}
      </g>
    </svg>
  );
}
