...
export const cotizarSeguroAutoAgentTool: AgentToolDefinition = {
  ...
  buildPromptBlock(ctx) {
    ...
    const technicalRule =
      "Si la herramienta responde con technical:true (sin importar si ok es true o false), es un fallo de backend (proveedor de datos, aseguradora no conectada o mal configurada, error de su API) — NUNCA se lo menciones al cliente ni digas nada que suene a problema técnico o de conexión: simplemente dile con calidez que ya tienes sus datos y que un asesor le va a confirmar el precio en breve, igual que cuando todo sale bien. El campo `reason` en ese caso es solo para tu referencia interna, no lo repitas.";
    return ctx.quotingRules.autoQuote
      ? `Tienes una herramienta (cotizar_seguro_auto) para cotizar seguros de auto de verdad. Pide la placa primero. ${preguntasVehiculo} Luego pide nombre completo, documento y fecha de nacimiento del tomador, de forma natural. Si la herramienta dice que faltan datos, pide exactamente esos. ${technicalRule} Nunca inventes ni aproximes una prima.`
      : `Tienes una herramienta (cotizar_seguro_auto) para REUNIR los datos de una cotización de auto (no te da el precio directo — eso lo confirma un asesor). Pide la placa primero. ${preguntasVehiculo} Luego pide nombre completo, documento y fecha de nacimiento del tomador, de forma natural. Cuando la herramienta confirme que los datos quedaron completos, dile al cliente que un asesor le va a confirmar el precio en breve — nunca inventes ni aproximes una prima tú mismo. ${technicalRule}`;
  },
