...
export async function syncWebhookTriggers(
  ...
  // Cuatro tipos de disparador tienen URL pública propia, cada uno con su campo de token
  // (ver TOKEN_FIELD_BY_TRIGGER_TYPE en node-types.ts) — todos comparten este mismo índice
  ...
  const rows = graph.nodes
    .filter(
      (n) =>
        ...
        (n.type === "trigger.woocommerce_order" &&
          typeof n.data.woocommerceOrderWebhookToken === "string" &&
          n.data.woocommerceOrderWebhookToken) ||
        (n.type === "trigger.woocommerce_product" &&
          typeof n.data.woocommerceProductWebhookToken === "string" &&
          n.data.woocommerceProductWebhookToken)
    )
    .map((n) => ({
      token: (n.type === "trigger.webhook"
        ? n.data.webhookToken
        : n.type === "trigger.hubspot_message"
          ? n.data.hubspotWebhookToken
          : n.type === "trigger.woocommerce_order"
            ? n.data.woocommerceOrderWebhookToken
            : n.data.woocommerceProductWebhookToken) as string,
