-- Módulo RBAC "seguros" (Noova Seguros: cotizador, renovaciones, siniestros,
-- cartera y comisiones para corredores/agencias). Habilitado por organización
-- vía organizations.settings.modules.seguros (ver src/lib/org-modules.ts) —
-- este módulo solo controla QUÉ puede hacer un usuario DENTRO de una
-- organización que ya tiene el flag encendido, no si la organización lo
-- tiene o no. Mismo patrón exacto que el módulo "erp" (102_erp_module.sql).

insert into public.permission_modules (key, label, description, scope, sort_order)
values ('seguros', 'Noova Seguros', 'Cotizador, renovaciones, siniestros, cartera y comisiones para corredores de seguros', 'organization', 78)
...
-- Backfill aditivo para organizaciones y plantillas ya existentes (no enciende
-- el módulo en sí, eso lo decide el flag por organización; solo deja listos
-- los niveles correctos para cuando se encienda).
...
-- Futuras organizaciones: incluir "seguros" en seed_organization_system_roles
-- con el mismo criterio que "erp" (manager/advisor = edit; owner/org_admin ya
-- heredan 'manage' porque iteran TODOS los módulos de scope organization;
-- viewer cae en 'view' por su rama por defecto).
...
begin
  ...
  select r_manager, key,
    case
      when key in ('voice_agents','text_agents','inbox','crm','channels','company_context','conectores','workflows','erp','seguros') then 'edit'::public.permission_level
  ...
  select r_advisor, key,
    case
      when key in ('inbox','crm','erp','seguros') then 'edit'::public.permission_level
